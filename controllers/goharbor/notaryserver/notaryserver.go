...
const (
	...
	DefaultConfigTemplatePath = "/etc/harbor-operator/templates/notaryserver-config.json.tmpl"
...
func New(ctx context.Context, configStore *configstore.Store) (commonCtrl.Reconciler, error) {
	...
	configStore.FileCustomRefresh(configTemplatePath, func(data []byte) ([]configstore.Item, error) {
		...
		logger.Get(ctx).WithName("controller").WithName(controllers.NotaryServer.String()).
	...
	r.Controller = commonCtrl.NewController(ctx, controllers.NotaryServer, r, configStore)
