...
Initially, we configured harbor by means of configmap, but currently we recommend using `harborconfiguration` crd to configure harbor, for the configmap method will be deprecated in version 2.3, those who have used and still use configmap will be automatically converted to `harborconfiguration` cr by the controller, and automatically remove old configmap.
