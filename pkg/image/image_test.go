...
var _ = Describe("Get image", func() {
	...
	Describe("Get image for in cluster redis", func() {
		It("Should pass", func() {
			...
			Expect(err).ToNot(HaveOccurred())
			Expect(image).To(Equal("redis:6.0-alpine"))
		})
	})
	...
	Describe("Get image for in cluster redis with repository", func() {
		It("Should pass", func() {
			...
			Expect(err).ToNot(HaveOccurred())
			Expect(image).To(Equal("ghcr.io/goharbor/redis:6.0-alpine"))
		})
	})
	...
	Describe("Get image for in cluster postgresql", func() {
		It("Should pass", func() {
			...
			Expect(err).ToNot(HaveOccurred())
			Expect(image).To(Equal("registry.opensource.zalan.do/acid/spilo-13:2.0-p7"))
		})
	})
	...
	Describe("Get image for in cluster postgresql with repository", func() {
		It("Should pass", func() {
			...
			Expect(err).ToNot(HaveOccurred())
			Expect(image).To(Equal("ghcr.io/goharbor/spilo-13:2.0-p7"))
		})
	})
})
