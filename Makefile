...
$(CHART_CRDS_PATH):
	ln -vs ../../config/crd/bases $(CHART_CRDS_PATH)

$(CHART_HARBOR_OPERATOR)/assets:
	ln -vs ../../config/config/assets $(CHART_HARBOR_OPERATOR)/assets
...
.PHONY:helm-generate
helm-generate: $(CHART_HARBOR_OPERATOR)/README.md $(CHART_CRDS_PATH) $(CHART_HARBOR_OPERATOR)/assets \
	$(CHART_HARBOR_OPERATOR)/charts $(CHART_HARBOR_OPERATOR)/Chart.lock \
